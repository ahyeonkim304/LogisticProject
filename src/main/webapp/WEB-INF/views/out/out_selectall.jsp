...
<head>
    ...
    <title>출고 관리 | Main Fulfillment</title>
</head>
<body>
        ...
        <div class="content-wrapper">
            ...
            <div class="page-header">
                <div class="list-title-block">
                    <h2>출고 리스트</h2>
                    <div class="subtitle">출고 정보를 조회하고 관리하는 화면입니다.</div>
                </div>
                <div class="flex gap-8">
                    ...
                    <a href="${ctx}/createPage" class="btn btn-primary">+ 출고 등록</a>
                </div>
            </div>

            <div class="card">
                <table class="data-table" id="outTable">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty outs}">
                                <tr><td colspan="9" class="table-empty">등록된 출고 데이터가 없습니다.</td></tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach items="${outs}" var="o">
                                    <tr>
                                        <td class="center"><input type="checkbox" name="rowCheck" value="${o.id}" /></td>
                                        <td class="center">${o.id}</td>
                                        <td><c:out value="${o.product.productCode}" default="-" /></td>
                                        <td><c:out value="${o.product.name}" default="-" /></td>
                                        <td class="center">${o.outStock}</td>
                                        <td>${o.outRequest_at}</td>
                                        <td>${o.outComplete_at}</td>
                                        <td class="center">
                                            <c:choose>
                                                <c:when test="${o.outStatus}"><span class="badge badge-success">완료</span></c:when>
                                        ...
                                        <td class="center">
                                            <form action="${ctx}/updatePage" method="post" style="display:inline;">
                                                ...
                                                <input type="hidden" name="outStock"  value="${o.outStock}" />
                                                <input type="hidden" name="outStatus" value="${o.outStatus}" />
                ...
                <div class="pagination">
                    ...
                    <div class="total-count">
                        Total <c:out value="${empty outs ? 0 : outs.size()}" default="0" />
                    </div>
                </div>
            </div>
        </div>
